...
data_whole <- subset(data_whole, Rejion == "강남구")  # 다른 구 분석하려면 이 부분 변경경
summary(data_whole)
```
```{r}
str(data_whole)
...
data_whole %>% 
  group_by(dong) %>% 
  summarise(transaction_real_price)
```
...
library(moments)
median(data_whole$unit_price); mean(data_whole$unit_price)
