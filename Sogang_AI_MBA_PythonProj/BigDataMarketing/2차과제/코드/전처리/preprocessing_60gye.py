...
# load data
df = pd.read_csv(r"C:\Users\luisf\OneDrive\바탕 화면\WorkSpace\Sogang_AI_MBA_Project\Sogang_AI_MBA_PythonProj\BigDataMarketing\2차과제\분석\1차 취합 및 분석\60계치킨 서울시 주문정보 분석\2차\combined_df_date수정2차.csv")
# print(df.head())
# data column 확인
print(df.columns)
...
grouped_df = df.groupby('region').apply(
    lambda x: x[['order_세분화1', 'order_세분화2', 'order_세분화3',
                 'order_세분화4', 'order_세분화5', 'order_세분화6']].apply(lambda row: [item for item in row if pd.notna(item)], axis=1).sum()
...
print(set(newlist))
# {' 더매운고추', '콤보', ' 고추', '장스', '육육', '양념', ' 짜장', '호랑이', ' 간지', '6초', '크크크', '사이드', '하하핫',
# '간지', ' ', ' 하하핫', ' 사이드', ' 육육', '빠삭', '고추', ' 크리스피', ' 양념', ' 빠삭', '골뱅이무침', ' 호랑이', ' 푸하핫',
# ' 크랑이', '짜장', '반마리 + 반마리', ' 6초', '크리스피', ' 크크크', '크랑이', ' 골뱅이무침', '후라이드', ' 콤보', ' 장스'}

# 공백 제거
cleaned_data = {item.strip() for item in newlist}
print(f"cleaned_data:{cleaned_data}")
# cleaned_data:{'', '장스', '육육', '양념', '푸하핫', '호랑이', '6초', '크크크', '사이드', '하하핫', '간지', '더매운고추',
# '빠삭', '후라이드', '고추', '골뱅이무침', '짜장', '반마리 + 반마리', '크리스피', '크랑이', '콤보'}
...
def plot_grouped_pie_charts(dataframe, region_column, flavor_column):
    ...
    for i in range(0, len(results), 3):
        ...
        plt.tight_layout()
        plt.savefig(f'60계 치킨 서울지역 pie_charts_row_{i // 3 + 1}.png', dpi=300, bbox_inches='tight')
        plt.close(fig)

plot_grouped_pie_charts(df, 'brand', '순살 여부')
#######################################################################################################################
# # 각 맛별로 매칭되는 아이템을 검색하여 맛 리스트를 생성
# # 지역명 데이터
# regions = [
...
#
#         if not flavor_counts.empty:
#             results.append((region, flavor_counts))
#
#     # 한 행에 3개씩 그리기
#     for i in range(0, len(results), 3):
...
#         for j, ax in enumerate(axs):
#             if i + j < len(results):
#                 region, counts = results[i + j]
#                 ax.pie(counts, labels=counts.index, autopct='%1.1f%%', startangle=90)
#                 ax.set_title(f'Pie Chart for {region}')
#             else:
#                 ax.axis('off')  # Turn off unused axes
#
#         plt.tight_layout()
#         plt.savefig(f'pie_charts_sunal_row_{i // 3 + 1}.png', dpi=300, bbox_inches='tight')
#         plt.close(fig)
#
