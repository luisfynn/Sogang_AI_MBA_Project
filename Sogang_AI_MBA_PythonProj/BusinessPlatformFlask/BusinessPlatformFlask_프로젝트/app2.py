...
@app.route('/')
def index():
    return render_template('indexProj.html')  # 처음에는 뉴스 데이터 없이 페이지 로드

@app.route('/fetch_news', methods=['POST'])
def fetch_news():
    print("run fetch_news")
    company = request.form.get('company_name', '부동산')  # 기본적으로 '부동산' 검색
    news_list = get_news_list(company)
    print(f"news_list:{news_list}")
    return jsonify(news_list)  # 결과를 JSON 형식으로 반환
