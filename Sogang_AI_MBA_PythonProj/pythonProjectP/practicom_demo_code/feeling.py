...
# json_dir = r'C:/Users/LG/practicumfeeling/feeling'
#
# # 클래스 매핑
# label_mapping = {'happy': 0, 'others': 1}
#
...
#
# # 폴더 정의
# train_dir = r'C:/Users/LG/practicumfeeling/feeling/train'
# val_dir = r'C:/Users/LG/practicumfeeling/feeling/val'
...
# YOLO 학습 실행
# 경로 주의
# python yolov5/train.py --img 640 --batch 16 --epochs 50 --data "C:/Users/LG/Practicumfeeling/feeling/dataset.yaml" --weights yolov5s.pt
# --> 완료시 runs/exp/weights 폴더 아래 best.pt, last.pt 파일 생성됨
########################################################################################################################
# 학습된 라벨 확인:class(label)이 한글로 정상 출력되는지 확인
import torch

model = torch.hub.load('ultralytics/yolov5', 'custom', path= r'yolov5feeling/runs/train/exp4/weights/best.pt')
...
# 이미지 로드 및 예측
img = r'C:\python\pythonProjectP\uploads\faceUpload\upload_face.jpg'  # 예측할 이미지의 경로
resultsface = model(img)
print(f"results: {resultsface}")
...
# 결과 출력 및 클래스 이름 검증
resultsface.print()
for det in resultsface.xyxy[0]:  # per image
...
labelConverter = {'happy':'happy', 'others':'other feeling'}
