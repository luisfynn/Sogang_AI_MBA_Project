...
#######################################################################################################################
# 스쿼트
def squartFindAngle(image, kpts, p1,p2,p3, draw= True):
    coord = []
    no_kpt = len(kpts)//2
    ...
    return int(angle)
#######################################################################################################################
def squartPlotKpts(im, kpts, steps, orig_shape=None):
...
#######################################################################################################################
def squartProcessVideo(video_path, output_path, task_id):
    # YOLOv8 포즈 모델 로드
        ...
        for result in results:
            ...
            if boxes is not None and keypoints is not None:
                ...
                for idx in nms_indices:
                    ...
                    # 골격과 키포인트를 프레임에 그리기
                    squartPlotKpts(frame, xy_values.flatten(), steps=2)
                    ...
                    angle = squartFindAngle(frame, xy_values.flatten(), 12, 14, 16, draw=False)
                    print(f'angle: {angle}')
