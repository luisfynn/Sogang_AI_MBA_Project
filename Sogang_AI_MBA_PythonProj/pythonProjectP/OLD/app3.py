...
#######################################################################################################################
# 수정
UPLOAD_FOLDER = f'/home/ubuntu/upload/defaultUpload'
PUSHUP_UPLOAD_FOLDER = f'/home/ubuntu/upload/pushUpUpLoad'
SQUART_UPLOAD_FOLDER = f'/home/ubuntu/upload/squartUpLoad'
FACE_UPLOAD_FOLDER = f'/home/ubuntu/upload/faceUpLoad'
...
@app.route('/')
def index():
    return render_template('p_mainPage.html')
...
def register():
        ...
        return render_template('p_registerPage.html')
...
def schedule_file():
    ...
    return render_template('p_schedule.html', data=schedule_data)
...
def challenge_detail(challenge_id):
    ...
    return render_template('p_challenge_detail.html', challenge_info=challenge_info)
...
def my_page():
    ...
    return render_template('p_my_page.html')

@app.route('/challenge11')
@login_required
def challenge11():
    # 여기에서 11challenge.html 페이지에 필요한 데이터를 처리
    return render_template('p_11challenge.html')
...
def diary():
    ...
    return render_template('p_diary.html', entries=entries)
...
def face_emotion():
    if request.method == 'POST':
        ...
        if file:
            ...
            detection_result = face_detector(filepath)
            emotion = str(detection_result)
            new_entry = FaceEntry(content=filename, emotion=emotion)
            db.session.add(new_entry)
            db.session.commit()
            flash('Face entry added successfully.')
            return redirect(url_for('face_emotion'))

    entries = FaceEntry.query.all()
    return render_template('p_uploadFaceEmotion.html', entries=entries, face_upload_folder=app.config['FACE_UPLOAD_FOLDER'])
...
def create_challenge():
        ...
        return render_template('p_11challenge.html', recommended_users=recommended_users)
...
def view_challenges():
    ...
    return render_template('p_view_challenges.html', received_challenges=received_challenges,
