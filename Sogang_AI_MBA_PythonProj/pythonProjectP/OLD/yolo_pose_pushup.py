...
#######################################################################################################################
# pushup
def pushUpFindAngle(image, kpts, p1,p2,p3, draw= True):
    coord = []
    no_kpt = len(kpts)//2
    ...
    return int(angle)
#######################################################################################################################
def pushUpPlotKpts(im, kpts, steps, orig_shape=None):
...
#######################################################################################################################
def pushUpProcessVideo(video_path, output_path, task_id):
    # YOLOv8 포즈 모델 로드
    model = YOLO("trained_yolov8x-pose-p6.pt")
    cap = cv2.VideoCapture(video_path)
    subject = video_path.split('.')[0]
    ...
    # 출력 비디오 파일 설정
    # fourcc = cv2.VideoWriter_fourcc(*"mp4v") #DIVX, XVID, FMP4, X264, MJPG
    fourcc = cv2.VideoWriter_fourcc(*"X264") # modify
        ...
        for result in results:
            ...
            if boxes is not None and keypoints is not None:
                ...
                for idx in nms_indices:
                    ...
                    # 골격과 키포인트를 프레임에 그리기
                    pushUpPlotKpts(frame, xy_values.flatten(), steps=2)
                    ...
                    if H_SIDE == 1:
                        # 왼팔 각도 측정
                        angle_arm = pushUpFindAngle(frame, xy_values.flatten(), 9, 7, 5, draw=False)
                        print(f'H_SIDE:{H_SIDE} angle_arm: {angle_arm}')
                        angle_arm += 360
                    else:
                        # 오른팔 각도 측정
                        angle_arm = pushUpFindAngle(frame, xy_values.flatten(), 6, 8, 10, draw=False)
                        print(f'H_SIDE:{H_SIDE} angle_arm: {angle_arm}')

                    # 각도 업데이트: 각도 이상치 삭제(증가해야하는 구간에서 팔떨림 등으로 인해 측정값이 감소하는 이상값 삭제)
                    if direction == 0 and angle_arm < init_angle:
                        init_angle = angle_arm
                    elif direction == 1 and angle_arm > init_angle:
                        init_angle = angle_arm

                    # percent 표기
                    percentage = np.interp(init_angle, (MIN_ANGLE, MAX_ANGLE), (100, 0))  # angle_arm의 각도로 퍼센트 측정
                    bar = np.interp(init_angle, (MIN_ANGLE, MAX_ANGLE), (200, fh - 300))
                    print(f'angle_arm: {init_angle}, percentage: {percentage}, bar: {bar}')
    ...
    angle_df = pd.DataFrame(
        {"direction": direction_list, "angle": angle_list, "percent_list": percent_list, "init_angle": initangle_list})
    return angle_df